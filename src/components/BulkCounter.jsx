...
export default function BulkCounter({
  initialQty = 100,
  basePrice = 450,
  productName = "Product",
  availableColors = [],
  ...
  const safeColorString =
    typeof selectedColor === "string" ? selectedColor : "";
  const variantText = safeColorString
    ? `• Chosen Variant: ${safeColorString.toUpperCase()}`
    : "• Chosen Variant: Default/Standard";

  // Computes the absolute URL address link of the product photo asset
  const fullProductImageLink = productImage.startsWith("http")
    ? productImage
    : `https://venturesolutions.in${productImage}`;
  ...
  const handleEmailSubmission = (e) => {
    if (!clientEmail.trim() || !clientEmail.includes("@")) {
      e.preventDefault();
      setFormError(
        "⚠️ A valid business email address is required to submit a formal quote inquiry.",
      );
    } else {
      setFormError("");
    }
  ...
  const cleanDescription =
    productDesc && typeof productDesc === "object"
      ? productDesc.children
  ...
  const handleQuantityEnforcement = (value) => {
    ...
    if (!numericValue || numericValue <= initialQty) {
      ...
      const roundedMultiples =
        Math.ceil(numericValue / initialQty) * initialQty;
                  ...
                  <div className="modal-scroll-desc">
                    ...
                    <div
                      style={{
                        ...
                        lineHeight: "1.5",
                        whiteSpace: "pre-wrap",
                        borderLeft: "3px solid #0A3D33",
                        paddingLeft: "15px",
                      }}
                    >
                      <strong>Product Specification Details:</strong>
                      <br />
                      {cleanDescription || "Custom logo options available."}
                    </div>
                  </div>
                  ...
                  >
                    {availableColors && availableColors.length > 0 && (
                      <div style={{ marginBottom: "10px" }}>
                        ...
                        >
                          ...
                          <span style={{ color: "#0A3D33" }}>
                            {safeColorString.toUpperCase()}
                          </span>
                        </label>
                        <div style={{ display: "flex", gap: "8px" }}>
                          {availableColors.map((color) => {
                            const hexColor =
                              color === "tan" ? "#d2b48c" : color;
                    ...
                    >
                      ...
                      <input
                        ...
                        onBlur={(e) =>
                          handleQuantityEnforcement(e.target.value)
                        }
                        style={{
                          padding: "6px",
                          width: "95px",
                    ...
                    >
                      ...
                      >
                        Corporate Email RFQ Parameters (Email is Required):
                      </span>
                      <input
                        type="email"
